...
REFORMA/
  ...
  static/
    input.css               # Tailwind v4 entry point (@plugin "daisyui", @source directives)
    tailwind.css            # Compiled CSS output (generated by `npm run build:css`)
    style.css               # Custom styles
    script.js               # Vanilla JS (no framework)
  templates/                # Single-page Jinja2 template (DaisyUI "cupcake" theme)
  tests/                    # pytest test suite
```

**Frontend stack:** Tailwind CSS v4 + DaisyUI v5 (cupcake theme). CSS is compiled at build time via `@tailwindcss/cli` — no CDN in production. The compiled `static/tailwind.css` is committed so the Python backend has zero Node.js dependency at runtime.
